...
const Plan = require("../model/plan"); // ✅ adjust if needed
const Services = require("../model/services"); // ✅ adjust path if needed
...
const normalize = (str = "") =>
  String(str)
    .trim()
    .toLowerCase()
    // remove any "(...)" e.g. "X (formerly Twitter)" -> "X"
    .replace(/\(.*?\)/g, "")
    .replace(/\s+/g, " ")
    .trim();

// Your original platforms array (UNCHANGED pricing data)
const platforms = [
  {
    name: "threads",
    status: "Active",
...
async function seed() {
  try {
    ...
    // 1) Load all services to map subServiceHeading -> ids
    const services = await Services.find({}, { serviceId: 1, subServices: 1 }).lean();

    const subMap = new Map(); // normalizedHeading -> {serviceId, subServiceId}
    for (const svc of services) {
      const subs = Array.isArray(svc.subServices) ? svc.subServices : [];
      for (const ss of subs) {
        const key = normalize(ss.subServiceHeading);
        if (!key) continue;
        subMap.set(key, { serviceId: svc.serviceId, subServiceId: ss.subServiceId });
      }
    }

    // 2) Attach ids to each plan by matching Plan.name -> subServiceHeading
    const plansWithIds = platforms.map((p) => {
      const lookupKey = normalize(p.name); // "instagram" "youtube" etc
      const match = subMap.get(lookupKey);

      if (!match) {
        console.warn(`⚠️ No matching subService for plan "${p.name}". subServiceId will remain null.`);
        return { ...p, serviceId: null, subServiceId: null };
      }

      return {
        ...p,
        serviceId: match.serviceId,
        subServiceId: match.subServiceId,
      };
    });
    ...
    const inserted = await Plan.insertMany(plansWithIds);
    console.log(`✅ Seeded ${inserted.length} plans with subServiceId mapping`);
